-- Sample Data Insertion Script for Apartment Book Club
-- Run this AFTER running reset_database.sql and comprehensive_database_schema.sql
-- This provides test data to work with during development
...
-- First, create sample users in auth.users table (for testing purposes only)
...
INSERT INTO auth.users (
    ...
    ('00000000-0000-0000-0000-000000000000', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', 'authenticated', 'authenticated', 'john@example.com', '$2a$10$dummy.hash.for.testing.purposes', NOW(), NOW(), NOW(), '{"provider": "email", "providers": ["email"]}', '{"username": "john_doe"}', false, '', '', '', ''),
    ('00000000-0000-0000-0000-000000000000', 'bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb', 'authenticated', 'authenticated', 'jane@example.com', '$2a$10$dummy.hash.for.testing.purposes', NOW(), NOW(), NOW(), '{"provider": "email", "providers": ["email"]}', '{"username": "jane_smith"}', false, '', '', '', ''),
    ('00000000-0000-0000-0000-000000000000', 'cccccccc-cccc-cccc-cccc-cccccccccccc', 'authenticated', 'authenticated', 'mike@example.com', '$2a$10$dummy.hash.for.testing.purposes', NOW(), NOW(), NOW(), '{"provider": "email", "providers": ["email"]}', '{"username": "mike_wilson"}', false, '', '', '', ''),
    ('00000000-0000-0000-0000-000000000000', 'dddddddd-dddd-dddd-dddd-dddddddddddd', 'authenticated', 'authenticated', 'sarah@example.com', '$2a$10$dummy.hash.for.testing.purposes', NOW(), NOW(), NOW(), '{"provider": "email", "providers": ["email"]}', '{"username": "sarah_jones"}', false, '', '', '', '');

-- Now insert sample user details (these reference the auth.users we just created)
...
INSERT INTO books (id, title, author, description, category, rating, pages, isbn, publisher, lender_id, apartment_id) VALUES
    ...
    ('b0000002-0000-0000-0000-000000000002', 'Project Hail Mary', 'Andy Weir', 'A lone astronaut must save the earth by solving an impossible scientific mystery.', 'Science Fiction', 4.7, 476, '9780593135204', 'Ballantine Books', 'aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa', '11111111-1111-1111-1111-111111111111'),
